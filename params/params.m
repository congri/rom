...
if ~loadOldConf
    ...
    elseif strcmp(rom.mode, 'useLocalNeighbor')
        nNeighbors = 12 + 8*(rom.coarseMesh.nElX - 2) + 8*(rom.coarseMesh.nElY - 2) +...
    ...
    elseif strcmp(rom.mode, 'useLocalDiagNeighbor')
        nNeighbors = 16 + 12*(rom.coarseMesh.nElX - 2) + 12*(rom.coarseMesh.nElY - 2) +...
    ...
    elseif strcmp(rom.mode, 'global')
        rom.theta_c.theta = zeros(rom.fineMesh.nEl*rom.coarseMesh.nEl/prod(wndw), 1); %wndw is set in genBasisFunctions
    end
end
...
MCMC.Xi_start = conductivityTransform(.1*rom.conductivityTransformation.limits(2) +...
    .9*rom.conductivityTransformation.limits(1), rom.conductivityTransformation)*ones(rom.coarseMesh.nEl, 1);
...
stepWidth = 2e-0;
MCMC.randomWalk.proposalCov = stepWidth*eye(rom.coarseMesh.nEl);   %random walk proposal covariance
